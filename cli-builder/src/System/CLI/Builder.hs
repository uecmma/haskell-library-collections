...
buildCLIApp
  ...
  -> m a
buildCLIApp info middleware commonParser commands
  = join $ buildCLI info Nothing parser
  where
...
buildCLIGeneralApp
  ...
  -> CommandExecutes c a
  -> m a
buildCLIGeneralApp info commonParser mOnFailure commands
  = buildCLI info mOnFailure parser
  where
...
buildSimpleCLI
  ...
  -> (c -> m a)
  -> m a
buildSimpleCLI info parser cmd = join $ buildCLI info Nothing $ cmd <$> parser
